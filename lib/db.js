...
var handleUpdate = function(channel, message) {
    ...
    try {
        ...
        console.log("Invalid meessage: " + message); 
        return;
    }
    if (message.clientID === 'nodejs') {
        return;
    }
...
var init = function(options) {
    ...
    // Pipe receives binary strings
    var pipe_opts = extend({return_buffers: true}, options);
    pipe = options.pipe ||
           redis.createClient(options.port,options.host,pipe_opts);
...
var apiHandlers = {
    ...
    create: function(req, res, next) {
        ...
        var id = req.id;
        delete req.id;
        if (req.broadcast === undefined || req.broadcast)
